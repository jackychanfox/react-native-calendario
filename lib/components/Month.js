...
class WeekColumns extends React.Component<{
  ...
  renderWeekText = (day: string, i: number) => (
    ...
    >
      {this.props.renderWeekContent?this.props.renderWeekContent(day, i):<Text
        allowFontScaling={false}
...
export default class Month extends React.Component<PropsType> {
  ...
  render() {
    ...
    return (
      <View style={[{ height: height+(weeks.length==6?20:0) }, theme.monthContainer]}>
        <View style={{flex:1, justifyContent:'center'}}>
          {showMonthTitle && <MonthTitle name={name} theme={theme} />}
          {showWeekdays && <WeekColumns days={DAY_NAMES} theme={theme} renderWeekContent={renderWeekContent} />}
