...
class NonTouchableDay extends React.Component<NonTouchableDayPropsType> {
  ...
  render() {
    const { isMonthDate, isActive, isOutOfRange, theme, date, isUnavailable } = this.props;
    ...
    return (
      <View
        style={[
          ...
          isUnavailable? theme.unavailableContainerStyle:{}
        ]}
      >
        <Text
          style={[
...
export default class Day extends React.Component<PropsType, {}> {
  ...
  isUnavailable(date){
    ...
    if (disabledDays.indexOf(D)>=0) return true
  }
  ...
  render() {
    ...
    const isUnavailableDate = this.isUnavailable(date)
    if (!isVisible||isUnavailableDate) {
      return (
        <NonTouchableDay
          isUnavailable={isVisible && isUnavailableDate}
          isActive={isActive}
          date={date}
          theme={theme}
